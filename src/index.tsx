...
class RichMarkdownEditor extends React.PureComponent<Props, State> {
  ...
  createNodeViews(): NodeViewDict {
    return this.extensions.extensions
      .filter((extension: ReactNode) => extension.component)
      .reduce((nodeViews, extension: ReactNode) => {
        return {
          ...nodeViews,
          [extension.name]: (node, view, getPos, decorations) => {
  ...
  createCommands(): { [name: string]: (attrs: any) => void } {
  ...
  createParser(): MarkdownParser {
    return this.extensions.parser({ schema: this.schema })
  }
  ...
  createView(): EditorView {
    ...
    const isEditingCheckbox = (tr: Transaction) => {
      return tr.steps.some(
        (step: Step) =>
          (step as any).slice.content.firstChild &&
          (step as any).step.slice.content.firstChild.type.name ===
  ...
  getHeadings = (): LocalHeading[] => {
    const headings: LocalHeading[] = []
