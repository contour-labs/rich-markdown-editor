...
import { ConflictIdentity } from '../nodes/MergeConflict'
...
export const regexParseConflicts = (
  ...
  while ((matches = conflictSeeker.exec(fileContents)) !== null) {
    ...
    const intermediateStartIndex = !partitions.length
      ? 0
      : (partitions[partitions.length - 1] as MergeConflict).endIndex
    const intermediateEndIndex = startIndex
    if (intermediateStartIndex !== intermediateEndIndex) {
      partitions.push(
        fileContents.slice(intermediateStartIndex, intermediateEndIndex)
      )
    }
  ...
  if (partitions.length) {
    const startIndex = (partitions[partitions.length - 1] as MergeConflict)
      .endIndex
...
export const documentWithConflicts = (
  markdownParser: MarkdownParser,
  results: RegexParseResults,
  nodeDict: NodeDict
): Node => {
...
export const resolveConflict = (
  ...
  const output =
    contents.slice(0, startIndex) + resolvedContent + contents.slice(endIndex)
