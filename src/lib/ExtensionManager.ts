...
export default class ExtensionManager {
  ...
  parser({ schema }: { schema: Schema }): MarkdownParser {
    const tokens = this.extensions
      ...
      .reduce((nodesAccumulator, extension: Node | Mark) => {
        const tokenConfig: TokenConfig | undefined = extension.parseMarkdown()
        if (tokenConfig) {
          nodesAccumulator = {
            ...nodesAccumulator,
            [extension.markdownToken || extension.name]: tokenConfig,
          }
        }
  ...
  get marks(): { [name: string]: MarkSpec } {
    return this.extensions
      .filter(extension => extension.type === 'mark')
      .reduce(
  ...
  commands({ schema, view }) {
    return this.extensions
      ...
      .reduce((allCommands, extension) => {
        ...
        const handle = (_name: string, _value: any[]) => {
